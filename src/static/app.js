    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0 
          ? details.participants.map(p => `<li class="participant-item"><span class="participant-name">${p}</span><button class="delete-participant" type="button" data-activity="${name}" data-participant="${p}" aria-label="Remove ${p}">✕</button></li>`).join('') 
